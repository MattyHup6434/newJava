...
public class RegisterService {
    ...
    public Integer save(RegisterRequestModel request) {
        ...
        if (null != request) {
        	//ใช้ userRepository ค้นหาผู้ใช้งานในฐานข้อมูลโดยใช้ชื่อผู้ใช้งานจาก request.getUserName().
            UserEntity entity = userRepository.findByUserName(request.getUserName());
            //การตรวจสอบว่าผู้ใช้งานมีอยู่หรือไม่
            if(null != entity) {
                return null;
            }
            ...
            userEntity.setCreateBy("Bill");
            userEntity.setCreateDate(new Date());
            userEntity.setUpdateBy("Nawaphon");
            ...
            if (null != userEntity) {
                ...
                userDetailEntity.setCreateBy("Bill");
                userDetailEntity.setCreateDate(new Date());
                userDetailEntity.setUpdateBy("Nawaphon");
    ...
    public Integer update(RegisterRequestModel request) {
        ...
        if (null != request) {
            ...
            if (userEntity.isPresent()) {
                ...
                response = user.getId();
                user.setUserName(null != request.getUserName() ? request.getUserName() : user.getUserName());
                ...
                UserDetailEntity userDetail = userDetailRepository.findByUserId(request.getUserId());

                if (null != userDetail) {
                    userDetail.setFristName(
                            null != request.getFristName() ? request.getFristName() : userDetail.getFristName());
                    userDetail.setLastName(
                            null != request.getLastName() ? request.getLastName() : userDetail.getLastName());
